...
app.get('/status-success', (req, res) => {
    ...
    exec('/home/ec2-user/my-api/success_script.sh', (err, stdout, stderr) => {
        if (err) {
            console.error(err);
...
app.get('/status-failed', (req, res) => {
    ...
    exec('/home/ec2-user/my-api/failed_script.sh', (err, stdout, stderr) => {
        if (err) {
            console.error(err);
            return res.status(500).send('Internal Server Error');
        }
...
// http://18.142.46.49:3000/status-success
